...
void Defe_Init(void)
{
  ...
  C_BIT(DEFE_Base_Address+0X0,31);//set to 0 when using DEFE
	...
	//开启中断	
	// DE_SCAL_EnableINT(0,Wb_end);//回写结束中断
	// request_irq(IRQ_DEFE, IRQ_DEFE_ISR , 0);
}
/*********************************************************************************************
DEFE配置
...
void Defe_Config_video_uvcombined_yuv422_to_argb(int inx,int iny,int outx,int outy)
{
	//-------------------------输入
...
//	DE_SCAL_Writeback_Enable(0);
  DE_SCAL_Set_CSC_Coef(0,csc_bt601,csc_bt601,csc_YUV,csc_RGB,1,0);//颜色转换系数 已改BR SWAP
...
/***********************************************************************************
配置图层缩放, PS: 输入RB颜色需手动对调
***********************************************************************************/
extern u32 *defe_buf_addr;
void Defe_Config_layer_scale(int inx, int iny, int outx, int outy)
{
    //-------------------------输入
    __scal_buf_addr_t src_buf_addr;
    __scal_scan_mod_t src_scan;
    ...
    src_scan.field = 0;  //0:frame scan; 1:field scan
    src_scan.bottom = 0; //0:top field; 1:bottom field
    src_buf_addr.ch0_addr = (u32)defe_buf_addr;
    ...
    //	DE_SCAL_Set_Writeback_Addr(0,&backaddr);
    //	DE_SCAL_Writeback_Enable(0);
    // DE_SCAL_Set_CSC_Coef(0, csc_bt601, csc_bt601, csc_RGB, csc_RGB, 0, 0); //颜色转换
    DE_SCAL_Set_Scaling_Factor(0, &src_scan, &src_size, &src_type, 
                                &out_scan, &out_size, &out_type);
    DE_SCAL_Set_Init_Phase(0, &src_scan, &src_size, &src_type, 
                            &out_scan, &out_size, &out_type, 0);
    DE_SCAL_Set_Scaling_Coef(0, &src_scan, &src_size, &src_type, 
                            &out_scan, &out_size, &out_type, 0);
    DE_SCAL_Set_Scaling_Coef_for_video(0, &src_scan, &src_size, &src_type, 
                                        &out_scan, &out_size, &out_type, 0);
    DE_SCAL_Set_Out_Format(0, &out_type);
    DE_SCAL_Set_Out_Size(0, &out_scan, &out_type, &out_size);
    //***********************************************************************************
    S_BIT(DEFE_Base_Address+0X08,1); //当输入输出数据格式相同时置1
    // C_BIT(DEFE_Base_Address+0X08,0); //当输入输出大小相等时置1

    DE_SCAL_Output_Select(0, 0); //out<0:debe0;  3:writeback>//输出选择
    DE_SCAL_Set_Reg_Rdy(0);      //寄存器已设置好
}
/*********开启显示缩放**********/
void Defe_Disply_scale(int inx, int iny)
{
    Defe_Init();
    Defe_Config_layer_scale(inx, iny, fb_f1c100s.width, fb_f1c100s.height);
    Defe_Start();
}
...
// #ifdef	LCD_TYPE_RGB43_480_272
	...
	if(inmode==TVD_SOURCE_NTSC)
		Defe_Config_video_uvcombined_yuv422_to_argb(720,480,fb_f1c100s.width,fb_f1c100s.height);
	else	if(inmode==TVD_SOURCE_PAL)
		Defe_Config_video_uvcombined_yuv422_to_argb(720,576,fb_f1c100s.width,fb_f1c100s.height);	
#endif

#ifdef	LCD_TYPE_TV_NTSC_720_480	
	if(inmode==TVD_SOURCE_NTSC)	
		Defe_Config_video_uvcombined_yuv422_to_argb(720,480,720,240);
	else	if(inmode==TVD_SOURCE_PAL)
		Defe_Config_video_uvcombined_yuv422_to_argb(720,576,720,240);
#endif	
	
#ifdef	LCD_TYPE_TV_PAL_720_576	
	if(inmode==TVD_SOURCE_NTSC)	
	 Defe_Config_video_uvcombined_yuv422_to_argb(720,480,720,576);
	else	if(inmode==TVD_SOURCE_PAL)
	 Defe_Config_video_uvcombined_yuv422_to_argb(720,576,720,576);
#endif	
}
